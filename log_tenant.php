...
<main>     
    <div class="log-table anim t1">
        <div class="logbtn">
            <a href="logs.php#Tenant" class="buttonlogr">Return to Brief Tenant Log</a>
        </div>
    ...
    $query = "SELECT log_tbl.*, log.*, 
            IFNULL(tenant_tbl.first_name, log_tbl.l_first_name) AS tenant_name
            FROM `log_tbl`
            LEFT JOIN tenant_tbl ON log_tbl.tenant_id = tenant_tbl.tenant_id
            JOIN `log` ON log_tbl.log_type_id = log.log_type_id";
    
    if (!empty($filtervalues)) {
        $query .= " WHERE CONCAT(IFNULL(tenant_tbl.first_name, log_tbl.l_first_name), 
                                log_tbl.tenant_id, 
                                log_tbl.tenant_log_id, 
                                log_tbl.date_entry, 
                                log.log_type) LIKE '%$filtervalues%'";
    }

    $query .= " ORDER BY log_tbl.tenant_log_id DESC
    ...
    if (!empty($filtervalues)) {
        $count_query .= " WHERE CONCAT(IFNULL(tenant_tbl.first_name, log_tbl.l_first_name), 
                                        log_tbl.tenant_id, 
                                        log_tbl.tenant_log_id, 
                                        log_tbl.date_entry, 
                                        log.log_type) LIKE '%$filtervalues%'";
