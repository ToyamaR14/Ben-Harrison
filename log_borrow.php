...
<main>     
    <div class="log-table anim t1">
        <div class="logbtn">
            <a href="logs.php#Borrow" class="buttonlogr">Return to Brief Borrow Log</a>
        </div>
    ...
    if (!empty($filtervalues)) {
        $query .= " WHERE CONCAT(borrow_log_id, borrow_id, date_entry) LIKE '%$filtervalues%' ";
    }
    
    $query .= " ORDER BY borrow_log_id DESC
    ...
    $count_query = "SELECT COUNT(*) AS total 
                    FROM log_tbl_b";
    if (!empty($filtervalues)) {
        $count_query .= " WHERE CONCAT(borrow_log_id, borrow_id, date_entry) LIKE '%$filtervalues%'";
    }
